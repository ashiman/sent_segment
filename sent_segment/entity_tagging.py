...
SHORT_FORMS_REGEX = {
    ur'\b(acc|act)\b': ur'account',
    ur'\b(t\s*n\s*c|t\s*&\s*c)s?\b': ur'terms and conditions',
    ur'\b(min\*order)\b': ur'minimum order',
    ur'\b(XX+)(To)\b': ur'\1 \2',
    ur'\b(o\s*/\s*s)\b': ur'outstanding',
    ur'\b(avlbl|avl)\b': ur'available',
    ur"\b(appx|approx)\b": ur"approximately",
    ur'\b(txn|trxn|tcn)\b': ur'transaction',
    ur'\b(plz|pls)\b': ur'please',
    ur'\b(pvt\.?)\b': ur'private',
    ur'\b(ltd\.?)\b': ur'limited',
    ur'\b(pn?ts)\b': ur'points',
    ur'\b(regd\.?)\b': ur'registered',
    ur'\b(no\.|num)\s?[:\-]?\b': ur'number ',
    ur'\b(wef|w\.e\.f)\b': ur'with effect from',
    ur"\b(wi\s*fi|wi\-fi)\b": ur"WIFI",
    ur'\b(rd\.?)\b': ur"road",
    ur'\b(ref.?|reference)\s+(number|no.?|num.?)\b': ur'reference number ',
    ur'\b(e\.g\.)': u'for example',
    ur'\b(i\.e\.)': u'that is',
    ur'\b(etc)\b': u'and other things'
}
